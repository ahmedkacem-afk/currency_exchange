...
import { signIn as supabaseLogin } from "./supabase/auth"
...
import { getUserCustodyRecords as getUserCustodyRecordsFromSupabase } from "./supabase/tables/custody"
...
export async function setPrices(payload) {
  ...
  const formattedPayload = {
    buyPrice:
      payload.buyold !== undefined
        ? Number(payload.buyold)
        : payload.buynew !== undefined
          ? Number(payload.buynew)
          : payload.buyprice !== undefined
            ? Number(payload.buyprice)
            : 5.0,

    sellPrice:
      payload.sellold !== undefined
        ? Number(payload.sellold)
        : payload.sellnew !== undefined
          ? Number(payload.sellnew)
 ...
 *
 * @returns {Promise<Array>} - Array of custody records
 */
export async function getUserCustodyRecords() {
  console.log("API: Getting user custody records from Supabase")
  return await getUserCustodyRecordsFromSupabase()
}
