...
export async function createTreasuryWallet(treasurerId, treasurerName) {
  try {
    ...
    const { error: currencyError } = await supabase.from("wallet_currencies").insert([
      {
        wallet_id: walletId,
        currency_code: "USD",
        balance: 0,
    ...
    if (currencyError) {
      console.error("Wallet Custody Helpers: Error initializing treasury wallet currencies:", currencyError)
      // Don't throw - wallet was created successfully, just currencies failed
    }
