...
export async function updateCustodyBalance(custodyId, amount) {
  try {
    ...
    const { data: custody, error: getError } = await supabase
      .from('custody')
      .select('*')
      .eq('id', custodyId)
      .single();
    ...
    const newBalance = parseFloat(custody.amount) + parseFloat(amount);
...
export async function getFormattedCustodyOptions(currencyCode = null, excludeEmpty = true) {
  try {
    ...
    if (excludeEmpty) {
      filteredRecords = filteredRecords.filter(record => parseFloat(record.amount) > 0);
    }
  ...
  try {
    ...
    const walletOptions = wallets
      .filter(wallet => 
        ...
        parseFloat(wallet.currencies[currencyCode]) > 0)
      .map(wallet => ({
        label: `${wallet.name}: ${parseFloat(wallet.currencies[currencyCode]).toFixed(2)} ${currencyCode}`,
        value: `wallet:${wallet.id}`,
        currencyCode: currencyCode,
        balance: parseFloat(wallet.currencies[currencyCode]),
        type: 'wallet'
      }));
