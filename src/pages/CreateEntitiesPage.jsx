  ...
  async function onCreateUser(e) {
    ...
    try {
      ...
      // For other roles, show standard message
        ...
        show(
          `User "${user.name}" created successfully as a treasurer, but there was an issue creating the treasury wallet. Please check the console for errors.`,
          "warning",
        )
      } else {
          ...
          <Card>
            ...
            <CardBody className="space-y-2">
              <form onSubmit={onCreateWallet} className="space-y-3">
                ...
                <Input
                  placeholder="10000"
                  label={t("create.initUsd")}
                  value={wUsd}
                  onChange={(e) => setWUsd(e.target.value)}
                />
                <Input
                  placeholder="75000"
                  label={t("create.initLyd")}
                  value={wLyd}
                  onChange={(e) => setWLyd(e.target.value)}
                />
                ...
                <div className="flex space-x-2">
                  <Button variant="outline" onClick={addCurrencyField} type="button" className="mb-2 bg-transparent">
                    + Add Currency
                  </Button>
          ...
          <Card>
            ...
            <CardBody className="space-y-2">
              <form onSubmit={onCreateUser} className="space-y-3">
                ...
                <RoleSelector currentRoleId={uRole} onRoleChange={setURole} required={true} />

                <CreateEntityButton disabled={loading.user} type="submit" variant="success">
                  {loading.user ? "..." : t("create.create")}
                </CreateEntityButton>
              </form>
            </CardBody>
          </Card>
          <Card>
            ...
            <CardBody className="space-y-2">
              <form onSubmit={onCreateOperation} className="space-y-3">
                <Input
                  placeholder="Dealings / معاملات"
                  label={t("create.operationName")}
                  value={opName}
                  onChange={(e) => setOpName(e.target.value)}
                />
                <CreateEntityButton disabled={loading.op} type="submit" variant="success">
                  {loading.op ? "..." : t("create.create")}
                </CreateEntityButton>
              </form>
            </CardBody>
          </Card>
