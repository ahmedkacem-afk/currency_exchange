...
-- This migration adds support f    -- Allow treasurer to create custody records
CREATE POLICY custody_treasurer_insert ON cash_custody 
  FOR INSERT WITH CHECK (auth.uid() = treasurer_id);
  
-- Enable RLS for wallets table if not already enabled
ALTER TABLE wallets ENABLE ROW LEVEL SECURITY;cash custody management between treasurer and cashiers
...
CREATE TABLE IF NOT EXISTS notifications (
  ...
  action_payload JSONB, -- Additional data for action handling
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
